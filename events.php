...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'import_latest') {
    ...
    // 1. Get the official_venue_id and user's name for the selected venue
    ...
    $stmt_user_info = $pdo->prepare("SELECT default_driver_name, default_race_class FROM users WHERE id = ?");
    $stmt_user_info->execute([$user_id]);
    $user_defaults = $stmt_user_info->fetch(PDO::FETCH_ASSOC);
    $driver_name_to_process = $user_defaults['default_driver_name'] ?? '';
    $race_class_to_process = $user_defaults['default_race_class'] ?? 'Mini BL'; // Default if not set
            ...
            if ($data && !empty($data['latest_event']['event_name'])) {
                ...
                if ($event_date_for_db) {
                    ...
                    try {
                        ...
                        if ($stmt_check->fetch()) {
                            $message = '<div class="alert alert-warning">This event already exists. Import skipped.</div>';
                            $pdo->rollBack();
                        } else {
                            // Insert the new event with a NULL track_id for now
                            $stmt_event = $pdo->prepare("INSERT INTO race_events (user_id, event_name, event_date, track_id) VALUES (?, ?, ?, ?)");
                            $stmt_event->execute([$user_id, $event_name, $event_date_for_db, null]);
                ...
                $message = '<div class="alert alert-warning">Scraper ran, but returned no new event data.</div>';
            }
        } else {
            $message = '<div class="alert alert-danger">Could not connect to the scraper API.</div>';
        }
    } else {
...
$stmt_events = $pdo->prepare("SELECT e.*, t.name as track_name, v.name as venue_name FROM race_events e LEFT JOIN tracks t ON e.track_id = t.id JOIN venues v ON e.venue_id = v.id WHERE e.user_id = ? ORDER BY e.event_date DESC");
