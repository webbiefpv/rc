...
// Handle form submission to add a new tire set
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_set') {
    ...
    if (empty($set_name)) {
        ...
        $message = '<div class="alert alert-success">New tire set added to inventory successfully!</div>';
    }
}

// Fetch all tire sets for the user, including a count of how many times they have been used
$stmt = $pdo->prepare("
    SELECT 
        ti.*, 
        COUNT(tl.id) as run_count
        ...
        <table class="table table-striped">
            ...
            <tbody>
                <?php if (empty($tire_sets)): ?>
                    <tr><td colspan="7" class="text-center">No tire sets in inventory. Add one above.</td></tr>
                <?php else: ?>
                    <?php foreach ($tire_sets as $set): ?>
                        <tr>
                            ...
                            <td><?php echo $set['purchase_date'] ? date("M j, Y", strtotime($set['purchase_date'])) : 'N/A'; ?></td>
                            <td>
